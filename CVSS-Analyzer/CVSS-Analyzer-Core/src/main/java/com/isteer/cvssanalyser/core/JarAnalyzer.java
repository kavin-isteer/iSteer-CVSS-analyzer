...
public class JarAnalyzer {
	...
	public void collectEvidencesFromJar(List<DependencyModel> dependencies) {
		...
		for (DependencyModel dependency : dependencies) {
			...
			List<Evidence> manifestVendorEvidence = collectVendorEvidencesFromManifest(
			...
			List<Evidence> manifestProductEvidence = collectProductEvidencesFromManifest(
					dependency.getArtifact().getFile());
			if (manifestProductEvidence != null) {
				for (Evidence wrkEvidence : manifestProductEvidence) {
					dependency.addProductEvidences(wrkEvidence);
					// getLog().info("adding manifest product evidence: "+manifestProductEvidence);
				}
			}

			// manifest version evidence
			List<Evidence> manifestVersionEvidence = collectVersionEvidencesFromManifest(dependency.getArtifact().getFile());
			if (manifestVersionEvidence != null) {
				for(Evidence wrkEvidence:manifestVersionEvidence) {
				dependency.addVersionEvidences(wrkEvidence);
	 ...
	 * @return Evidence object with the best manifest match, or null if no match
	 *         found
	 */
	...
	private List<Evidence> checkManifestVendorAttributes(Attributes attributes) {
		...
		String[] vendorAttributes = { "Implementation-Vendor", "Bundle-Vendor", "Specification-Vendor", "Created-By",
				"Built-By" };
	 ...
	 * 
	 * @param jarFile The JAR file to analyze
	 * @return Evidence object with the best product match, or null if no match
	 *         found
	 */
	public List<Evidence> collectProductEvidencesFromManifest(File jarFile) {
		DependencyHintDao hintDao = new DependencyHintDao();
		DbUtil dbUtil = new DbUtil();
		...
		try (JarFile jar = new JarFile(jarFile); Connection connection = dbUtil.getConnection()) {
			...
			// 2. Get product hints from database (type='product' and
			...
			Attributes mainAttributes = manifest.getMainAttributes();
			List<Evidence> bestEvidence = checkProductAttributes(mainAttributes);
	...
	private List<Evidence> checkProductAttributes(Attributes attributes) {
		// Ordered by priority of product attributes
		String[] productAttributes = { "Implementation-Title", "Bundle-SymbolicName", // OSGi
	 ...
	 */
	public List<Evidence> collectVersionEvidencesFromManifest(File jarFile) {
		try (JarFile jar = new JarFile(jarFile)) {
			Manifest manifest = jar.getManifest();
			if (manifest == null) {
			...
			String[] versionAttributes = { "Implementation-Version", // Highest priority
					"Bundle-Version", // OSGi
			...
			for (String attr : versionAttributes) {
				...
				if (version != null && !version.trim().isEmpty()) {
					...
					evidence.setEvidenceType(EvidenceType.MANIFEST_ENTRY);
					evidence.setEvidenceTitle(attr);
	...
	private String normalizeVersion(String version) {
		...
		String normalized = version.trim().replaceAll("^[\"']|[\"']$", "").replaceAll("-(RELEASE|FINAL|GA)", ""); // Common
																													// version
																													// suffixes
