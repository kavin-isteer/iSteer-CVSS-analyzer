...
public class PomFileReader {
	/**
	 * Analyzes a MultipartFile (expected to be a pom.xml) and extracts its direct
	 * dependencies. Properties within the POM are *attempted* to be resolved for
	 * common scenarios.
	 *
	 * @param pomFile The uploaded pom.xml as a MultipartFile.
	 * @return A list of DependencyModel objects.
	 * @throws IOException              If there's an error reading the uploaded
	 *                                  file.
	 * @throws XmlPullParserException   If there's an error parsing the XML content.
	 * @throws IllegalArgumentException If the uploaded file is empty or not a valid
	 *                                  POM.
	 */
	public List<DependencyModel> analyzePomFile(byte[] pomFile) throws IOException, XmlPullParserException {

		MavenXpp3Reader reader = new MavenXpp3Reader();
		Model model = null;

		try (Reader fileReader = new InputStreamReader(new ByteArrayInputStream(pomFile))) {
		...
		if (mavenDependencies != null) {
			for (Dependency dep : mavenDependencies) {
				...
				// String classifier = resolveProperty(dep.getClassifier(),
				// effectiveProperties);
	...
	/**
	 * Simple utility to resolve properties like ${prop.name} from the given
	 * properties map. This is a basic implementation and doesn't handle complex
	 * expressions or transitive properties.
	 */
	private String resolveProperty(String value, Properties properties) {
		...
		// Handle common built-in project/pom properties that might not be in the map
		// yet (though we try to add them)
		...
		// You might need more sophisticated property resolution if your POMs use
		// complex expressions.
		return value;
	}
}
