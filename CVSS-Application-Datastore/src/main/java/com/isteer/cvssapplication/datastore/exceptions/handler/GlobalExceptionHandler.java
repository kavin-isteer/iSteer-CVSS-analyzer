...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ErrorMessageDTO> handleHttpMessageNotReadable(HttpMessageNotReadableException ex) {
		...
		return new ResponseEntity<>(new ErrorMessageDTO(CVSSEnum.INVALID_INPUT.getStatusCode(),
				StatusMessageUtil.getMessage(CVSSEnum.INVALID_INPUT)), HttpStatus.BAD_REQUEST);
	}
	...
	public ResponseEntity<ErrorMessageDTO> handleDataIntegrityViolation(DataIntegrityViolationException ex) {
		...
		if (message.contains("computers.device_id")) {
			return new ResponseEntity<>(new ErrorMessageDTO(CVSSEnum.COMPUTER_DEVICE_ID_EXISTS.getStatusCode(),
					StatusMessageUtil.getMessage(CVSSEnum.COMPUTER_DEVICE_ID_EXISTS)), HttpStatus.BAD_REQUEST);
		} else if (message.contains("applications.name")) {
			return new ResponseEntity<>(new ErrorMessageDTO(CVSSEnum.DUPLICATE_APPLICATIONS.getStatusCode(),
					StatusMessageUtil.getMessage(CVSSEnum.DUPLICATE_APPLICATIONS)), HttpStatus.BAD_REQUEST);
		}
		return new ResponseEntity<>(new ErrorMessageDTO(CVSSEnum.DATA_INTEGRITY_VIOLATION.getStatusCode(),
				StatusMessageUtil.getMessage(CVSSEnum.DATA_INTEGRITY_VIOLATION)), HttpStatus.BAD_REQUEST);
	}

	@ExceptionHandler(IllegalArgumentException.class)
	public ResponseEntity<ErrorMessageDTO> handleIllegalArgument(IllegalArgumentException ex) {
		logger.error("Illegal argument: {}", ex.getMessage());
		return new ResponseEntity<>(new ErrorMessageDTO(CVSSEnum.ILLEGAL_ARGUMENT.getStatusCode(),
				StatusMessageUtil.getMessage(CVSSEnum.ILLEGAL_ARGUMENT)), HttpStatus.BAD_REQUEST);
	}
	...
	public ResponseEntity<ErrorMessageDTO> handleDatabaseError(BadSqlGrammarException ex) {
		...
		return new ResponseEntity<>(new ErrorMessageDTO(CVSSEnum.INVALID_SQL_SYNTAX.getStatusCode(),
				StatusMessageUtil.getMessage(CVSSEnum.INVALID_SQL_SYNTAX)), HttpStatus.INTERNAL_SERVER_ERROR);
	}
	...
	public ResponseEntity<ErrorMessageDTO> handleJsonMappingException(JsonMappingException ex) {
		...
		return new ResponseEntity<>(new ErrorMessageDTO(CVSSEnum.COMPUTER_PAYLOAD_INVALID.getStatusCode(),
				StatusMessageUtil.getMessage(CVSSEnum.COMPUTER_PAYLOAD_INVALID)), HttpStatus.BAD_REQUEST);
	}
